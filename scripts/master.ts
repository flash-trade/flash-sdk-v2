...
async function run() {
    ...
    return logSent(
      await sendBase(
        ...
        await ctx.client.delegateBasket(owner, {
          commitFrequency: ENV.commitFrequencyMs,
          validatorKey: validatorKeyForCluster(ENV.cluster),
        }),
      ),
    );
  });
  ...
  // per-position views — getPositionData / getPnl / getExit* / getLiquidation* /
  // get{Add,Remove,Close}Quote — which derive a standalone findPositionAddress PDA,
  // do NOT apply to this pool (they'd 3012 on a non-existent account). The correct
  // way to read an ER position is to fetch the (delegated) basket.
  explain(
    positionOpen
      ? "ER positions live in the basket — reading it (per-position views are for the non-ER model)"
  ...
  await step("requestWithdrawal", async () => {
    ...
    const res = await ctx.client.withdrawalWithAction(
      ...
      ownerAta,
      withdrawAmount,
      { commitFrequency: ENV.commitFrequencyMs, validatorKey: validatorKeyForCluster(ENV.cluster) },
      payer.publicKey,
    );
