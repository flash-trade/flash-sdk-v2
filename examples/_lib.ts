...
export const ENV = {
  ...
  poolName: process.env.POOL || "devnet.1",
...
export function logSent<T extends Record<string, any>>(sent: T, indent = ""): T {
  if ((sent as any).dryRun) {
    console.log(
      `${indent}  (dry-run) built ${(sent as any).instructionCount} instruction(s) — set SEND=1 to submit`,
    );
  } else if ((sent as any).signature) {
    console.log(`${indent}  ✓ ${(sent as any).layer} tx on ${(sent as any).network}: ${(sent as any).signature}`);
    console.log(`${indent}    ${(sent as any).explorer}`);
  }
  return sent;
}
